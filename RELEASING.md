...
- Re-create the CLDF dataset:
  ```shell
  cldfbench makecldf --glottolog-version v5.2 cldfbench_bowern_and_atkinson2012.py --with-zenodo --with-cldfreadme
  ```
...
- Run Phlorest-specific checks on the data:
  ```shell
  phlorest check --with-R cldfbench_bowern_and_atkinson2012.py
  ```
...
- Create the release instruction:
  ```shell
  phlorest release cldfbench_bowern_and_atkinson2012.py vX.Y
  ```
